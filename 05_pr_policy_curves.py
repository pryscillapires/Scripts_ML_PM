...
"""
05_pr_policy_curves.py (improved v2)

OOF-only PR + policy curves for Patroclus–Menoetius stability surrogate.

This v2 keeps the previous improvements and adds:
- If unstable_selected_share is ~0 for all top-k points, we *hide* the secondary axis
  (avoids the confusing 1e-9 scale) and place a clear note on the plot.
...
def _confusion_at_threshold(y, p, thr):
    ...
    fp = int(np.sum((y == 0) & (pred == 1)))  # false stable
...
def choose_threshold_by_fpr(y, p, fpr_target):
    ...
    thr_candidates = np.unique(p[np.isfinite(p)])
    ...
    for thr in thr_candidates:
        ...
        if not (np.isfinite(fpr) and np.isfinite(rec)):
            continue
        if fpr <= fpr_target:
...
def plot_policy(policy_dict, title, outpath):
    ...
    ax.set_ylabel("Value")
    ax.set_xlim(0.0, max(x) * 1.02)
    ax.set_ylim(0.0, 1.02)
    ax.grid(True, alpha=0.3)
    ax.set_title(title)

    # If unstable share is essentially zero everywhere, hide the secondary axis
...
def main():
    ...
    ap.add_argument("--base", default="out/ml/surrogate")
    ...
    fracs = tuple(float(x) for x in args.topk.split(","))
    ...
    for mode in ["pro", "retro"]:
        ...
        tp, fp, tn, fn = _confusion_at_threshold(y, p, 0.5)
        prec, rec, fpr, tnr, acc = _rates_from_conf(tp, fp, tn, fn)
    ...
    with open(outdir / f"summary_pr_policy_{args.model}.json", "w") as f:
